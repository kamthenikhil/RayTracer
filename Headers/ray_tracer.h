...
class Plane;
class Render_World;
void Initialize_World(Render_World& world, const int, const int,
...
public:
	...
	static Vector_3D Cross_Product(const Vector_3D& v1, const Vector_3D& v2) // 6 mults, 3 adds
			{
		return Vector_3D(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z,
				v1.x * v2.y - v1.y * v2.x);
	}
...
public:
	...
	MinimumBoundingBox() :
			x_min(0), x_max(0), y_min(0), y_max(0), z_min(0), z_max(0), longestAxis(
					0) {
	}
};
...
public:
	...
	KdTreeNode(vector<Object*> objects) {
		this->objects = objects;
	}

	void setMinimumBoundingBox();
};
...
public:
	...
	KdTreeNode* build(vector<Object*> objects);
...
public:
	...
	Grid_2D(int m_input, int n_input, double xmin_input, double xmax_input,
			double ymin_input, double ymax_input) {
		Initialize(m_input, n_input, xmin_input, xmax_input, ymin_input,
				ymax_input);
	}
...
public:
	...
	Ray() :
			endpoint(0, 0, 0), direction(0, 0, 1), semi_infinite(true), t_max(
					0), current_object(0), recursion_depth(0) {
	}

	Ray(const Vector_3D<double>& endpoint_input,
			const Vector_3D<double>& direction_input) :
			endpoint(endpoint_input), direction(direction_input.Normalized()), semi_infinite(
					true), t_max(FLT_MAX),current_object(0),recursion_depth(0)
			{}
...
public:
	...
	Light(const Vector_3D<double>& position, const Vector_3D<double>& color,
...
public:
	Point_Light(const Vector_3D<double>& position,
			const Vector_3D<double>& color, const double brightness) :
...
public:
	...
	virtual Vector_3D<double> Shade_Surface(const Ray& ray,
			const Object& intersection_object,
			const Vector_3D<double>& intersection_point,
			const Vector_3D<double>& same_side_normal) const=0;

	static Vector_3D<double> Get_Reflected_Vector(
			const Vector_3D<double>& vector, const Vector_3D<double>& normal) {
		return normal * 2 * Vector_3D<double>::Dot_Product(vector, normal)
				- vector;
	}

	static Vector_3D<double> Get_Refracted_Vector(
			const Vector_3D<double>& vector, const Vector_3D<double>& normal,
			const double refractiveIndex1, const double refractiveIndex2) {
		double ratio = refractiveIndex1 / refractiveIndex2;
		double cosAngleOfIncidence = -Vector_3D<double>::Dot_Product(vector,
				normal);
		double cosAngleOfTransmittance =
				sqrt(
						1
								- (ratio * ratio
										* (1
		...
		return vector * ratio
				+ normal
						* (ratio * cosAngleOfIncidence - cosAngleOfTransmittance);
	}
...
public:
	...
	Phong_Shader(Render_World& world_input,
			...
			const Vector_3D<double>& color_specular = Vector_3D<double>(1, 1,
					1), const double specular_power = 50) :
			Shader(world_input), color_ambient(color_ambient), color_diffuse(
					color_diffuse), color_specular(color_specular), specular_power(
					specular_power) {
	}

	Vector_3D<double> Shade_Surface(const Ray& ray,
			const Object& intersection_object,
			const Vector_3D<double>& intersection_point,
			const Vector_3D<double>& same_side_normal) const;

	Vector_3D<double> Get_Ambient_Diffuse_Specular_Color(
			const Vector_3D<double>& intersection_point,
...
public:
	...
	Flat_Shader(Render_World& world_input, const Vector_3D<double>& color =
			Vector_3D<double>(1, 1, 1)) :
...
public:
	...
	Reflective_Shader(Render_World& world_input,
			...
			Phong_Shader(world_input, color_ambient, color_diffuse,
					color_specular, specular_power), reflectivity(
					std::min(1.0, reflectivity)) {
	}
...
// Refractive_Shader
//--------------------------------------------------------------------------------
class Refractive_Shader: public Reflective_Shader {
public:
	double refractiveIndex;

	Refractive_Shader(Render_World& world_input,
			const Vector_3D<double>& color_ambient,
			const Vector_3D<double>& color_diffuse,
			const Vector_3D<double>& color_specular = Vector_3D<double>(1, 1,
					1), const double specular_power = 50,
			const double reflectivity = 1, const double refractiveIndex = 1) :
			Reflective_Shader(world_input, color_ambient, color_diffuse,
					color_specular, specular_power, reflectivity), refractiveIndex(
					refractiveIndex) {
	}
	...
	double Get_Reflectance(const Vector_3D<double>& direction,
			const Vector_3D<double>& normal, const double refractiveIndex1,
			const double refractiveIndex2);
};
...
public:
	Sphere(const Vector_3D<double>& center_input, const double radius_input) :
		...
		Vector_3D<double> minimumBounds = getMinimumBounds();
		Vector_3D<double> maximumBounds = getMaximumBounds();
		minimumBoundingBox.x_min = minimumBounds.x;
		minimumBoundingBox.x_max = maximumBounds.x;
		minimumBoundingBox.y_min = minimumBounds.y;
		minimumBoundingBox.y_max = maximumBounds.y;
		minimumBoundingBox.z_min = minimumBounds.z;
		minimumBoundingBox.z_max = maximumBounds.z;
	}

	bool Intersection(Ray& ray) const;
	Vector_3D<double> Normal(const Vector_3D<double>& point) const;
	Vector_3D<double> getMinimumBounds() const;
	Vector_3D<double> getMaximumBounds() const;
};
...
public:
	...
	void Set_Resolution(const int pixels_width, const int pixels_height) {
		...
		pixel_grid.Initialize(pixels_width, pixels_height, -width * .5,
...
public:
	...
	void Position_And_Aim_Camera(const TV& position_input,
			const TV& look_at_point, const TV& pseudo_up_vector) {
		position = position_input;
		look_vector = (look_at_point - position).Normalized();
		horizontal_vector =
				TV::Cross_Product(look_vector, pseudo_up_vector).Normalized();
		vertical_vector =
				TV::Cross_Product(horizontal_vector, look_vector).Normalized();
	}

	void Focus_Camera(const double focal_distance, const double aspect_ratio,
			const double field_of_view) {
		focal_point = position + look_vector * focal_distance;
		film.width = (double) 2 * focal_distance
				* tan((double) .5 * field_of_view);
		film.height = film.width / aspect_ratio;
	}

	vector<Vector_3D<double> > World_Position(
...
//	std::map<long, Object*> objectMap;
	...
	Render_World() :
			background_shader(new Flat_Shader(*this, Vector_3D<double>())), enable_shadows(
					true), recursion_depth_limit(3) {
	}
	...
	bool intersection(MinimumBoundingBox minimumBoundingBox, Ray& ray);
	bool intersectionWithKdTreeNode(Ray& ray, KdTreeNode* node, double& t_max);
};
//--------------------------------------------------------------------------------
// Driver
//--------------------------------------------------------------------------------
...
public:
	...
	void Render_More(clock_t startTime, clock_t& endTime, bool& printed) {
		if (state_j >= Pixel_Height()) {
			if (state_j == Pixel_Height() && !printed) {
